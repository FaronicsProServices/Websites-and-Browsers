# =========================================================================================
# Script Name: Set-ChromeUserData-TDrive.ps1
# Description: Creates or updates the "Google Chrome" desktop shortcut to redirect 
#             chrome user data to the Thawspace.
# =========================================================================================

# Pre-flight check: Is the K: drive mapped and ready?
if (-not (Test-Path "K:\")) {
    Write-Warning "K: Drive not detected. Skipping shortcut modification to prevent Chrome errors."
    exit 0 # Exit 0 so the RMM doesn't show a 'Failure' alert, just a 'No Action' log.
}
...
$targetArguments = '--user-data-dir="K:\Chrome\User Data"'
...
try {
    ...
    Write-Output "Success: Shortcut at $destinationPath is now pointing to K:\ drive data."
} catch {
